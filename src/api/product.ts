 ...
 */
export async function getProductDetail(id: number): Promise<BaseResponse<ProductDetail>> {
  ...
  const response = await get<BaseResponse<RawProductDetail>>(`/api/products/${id}`)
  const adaptedData = adaptProductDetail(response.data)
...
const adaptProductDetail = (rawData: RawProductDetail): ProductDetail => {
  ...
  return {
    ...
    // 2. 核心转换逻辑：处理 SKUs，将英文键映射为中文键
    skus: rawData.skus.map((sku) => {
      const mappedSpecs: Record<string, string> = {}

      // 遍历后端给的英文键，映射为中文键
      for (const [englishKey, value] of Object.entries(sku.specs)) {
        const chineseKey = keyMapping[englishKey] || englishKey
        mappedSpecs[chineseKey] = value
      }
      ...
      return {
        ...
        price: sku.price,
        stock: sku.stock,
        diffParams: sku.diffParams,
        specs: mappedSpecs, // 使用映射后的中文键对象
      }
    }),
  }
}
